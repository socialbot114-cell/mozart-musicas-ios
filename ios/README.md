...
`StoreKit/Donation.storekit` espelha o Product ID e o tipo Não consumível. `python3 ios/Scripts/validate_catalog.py` compara esse contrato com o identificador usado pelo app antes do archive. A configuração local não comprova que o produto está disponível no App Store Connect; valide a compra real com um Apple ID sandbox em aparelho físico.
